...
const cache = {
  put: (req: { url?: string } | string, res: Response) =>
    res.text().then((body) => void store.set(typeof req === "string" ? req : (req.url ?? ""), body)),
...
Object.defineProperty(globalThis, "caches", { configurable: true, writable: true, value: {
  ...
  delete: () => Promise.resolve(true),
  match: (req: { url?: string } | string) => {
    const url = typeof req === "string" ? new URL(req, "https://radikal.wiki").href : (req.url ?? "");
...
async function purge() {
  for (const { url } of await cache.keys()) {
    ...
    if (pathname.startsWith("/assets/") || pathname.startsWith("/symbols/")) continue;
...
for (const [path, want] of reloads) {
  ...
  if (!ok) bad++;
  console.log(`  ${ok ? "ok  " : "FAIL"} ${path.padEnd(38)} -> ${got}${kept ? "" : "  (re-downloaded the hashed asset)"}`);
}
...
if (!doubles) bad++;
console.log(`  ${doubles ? "ok  " : "FAIL"} ${"without the purge, it takes two".padEnd(38)} -> ${first}, then ${second}`);
