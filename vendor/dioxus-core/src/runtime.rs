...
```"
    ...
    pub fn handle_event(self: &Rc<Self>, name: &str, event: Event<dyn Any>, element: ElementId) {
        ...
        // Read the path out and drop the borrow before calling user code, the
        // way `handle_bubbling_event` already does for `mounts`. Held across a
        // listener, it panics any render the listener triggers, in
