//! The OAuth callback is stateless, so the ephemeral link context (PKCE
//! verifier, the DPoP private key, the resolved endpoints, ...) must survive the
//! redirect to the PDS and back. It is serialized, encrypted (XChaCha20-Poly1305
//! with an HKDF-SHA256 key from `STATE_SECRET`) and set as a short-lived cookie
//! the browser carries to the callback.
//!
//! Transferred from the interim backend (`backend/src/statecookie.rs`). The one
//! real edit is the DID swap: the interim payload carried an `nhost_user_id` (the
//! NHost account being linked TO an atproto identity), but in the AppView the
//! atproto DID IS the identity, self-authenticated by the OAuth flow, so there is
//! no separate local account to bind. The `did` field is now the sole session
//! subject; the seal/open crypto is unchanged.
...
pub struct LinkState {
    pub handle: String,
    /// The account's DID: the self-authenticated session subject (replaces the
    /// interim `nhost_user_id`, since in the AppView the DID is the identity).
    pub did: String,
