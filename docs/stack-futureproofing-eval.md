...
| Realtime / sync transport | SurrealDB LIVE queries | keep-and-extend | The real change-feed is the atproto firehose, not a DB-vendor feature; binding realtime to SurrealDB LIVE re-creates the Hasura lock-in being unwound | Consume Jetstream filtered to `com.example.wiki.*`; push authoritative deltas over one axum WS fed by an in-process broadcast channel (the AppView is a single process holding Turso for both core and view plus the firehose, so no DB LISTEN/NOTIFY is needed) |
...
| atproto SDK (atrium) | not in README; hand-rolled OAuth | keep-and-extend | Maturest Rust atproto SDK (MIT, moved to the atrium-rs org); replaces the hand-rolled DPoP/PKCE | Adopt atrium-oauth/api/identity behind a thin wrapper; pin exact 0.25.x; validate `com.example.wiki.*` codegen early |
